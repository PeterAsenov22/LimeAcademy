...
contract CarTokenCrowdsale is CappedCrowdsale, MintedCrowdsale, RefundableCrowdsale {
  ...
  constructor(uint _startDate, uint _endDate, address _wallet, string _tokenName, string _tokenSymbol, uint8 _tokenDecimals) public
    CappedCrowdsale(HARD_CAP)
    RefundableCrowdsale(SOFT_CAP)
    TimedCrowdsale(_startDate, _endDate)
    Crowdsale(DEFAULT_RATE, _wallet, new CarToken(_tokenName, _tokenSymbol, _tokenDecimals))
    {
