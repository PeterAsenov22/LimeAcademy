...
contract Cars2 is ICars2, OwnableUpgradeableImplementation {
  ...
  function setOracleContractAddress(address _oracleContract) onlyOwner public {
  ...
  function getCarPriceInUSD(uint _index) public view onlyExistingCar(_index) returns (uint _carPriceInUSD) {
    ...
    return Car(cars[_index]).price().mul(ethPriceInUSD);
  }
